...
fixture="$atlas_root/sounio/src/metamorphic_fixture.sio"
...
echo "sounio_metamorphic_source_sha256=$fixture_sha256"

run_fixture() {
  local repo="$1"
  local root="$2"
  local output="/tmp/dosa-metamorphic-${expected_commit:0:12}-${fixture_sha256:0:12}.elf"

  cd "$repo"
  "$repo/bin/souc" --version
  echo "science_boundary=off (metamorphic executable fixture; not a release receipt)"
  "$repo/bin/souc" check "$root/sounio/src/metamorphic_fixture.sio" --science-boundary off
  "$repo/bin/souc" compile "$root/sounio/src/metamorphic_fixture.sio" -o "$output" --science-boundary off
  echo "metamorphic_executable_sha256=$(sha256_file "$output")"
  "$output"
}
...
if [[ -n "${SOUNIO_LIMA_INSTANCE:-}" ]]; then
  ...
  guest_fixture="/tmp/dosa-metamorphic-source-${fixture_sha256:0:12}.sio"
...
commit_short="$3"
fixture_short="$4"
output="/tmp/dosa-metamorphic-${commit_short}-${fixture_short}.elf"
...
echo "metamorphic_executable_sha256=$(sha256sum "$output" | awk '{print $1}')"
"$output"
SOUNIO_LINUX_RUNNER
else
