...
int main(int argc, char** argv) {
  if (argc != 2) {
    std::cerr << "usage: " << argv[0] << " <null_draws.xclbin>\n";
    return 2;
  }
  ...
  try {
    ...
    xrt::kernel kernel{context, "null_draws"};
    ...
    std::fill(outputs_map, outputs_map + kExpected.size(), -2147483647);
    ...
    for (std::size_t i = 0; i < kExpected.size(); ++i) {
      if (outputs_map[i] != kExpected[i]) {
        ...
        std::cerr << "U250_NULL_DRAW_MISMATCH case=" << item << " metric=" << metric
    ...
    std::cout << "U250_NULL_DRAW_PASS cases=" << kCaseCount
              << " metrics=" << kMetricCount
              << " replicates=" << kReplicates
              << " draws=" << kExpected.size() << "\n";
    return 0;
  } catch (const std::exception& error) {
    std::cerr << "U250_NULL_DRAW_ERROR " << error.what() << "\n";
    return 1;
  }
}
