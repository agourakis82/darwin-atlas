...
**Base commit:** the chromosome-benchmark commit updating this file on branch
`codex/sounio-primary-wip` (previous published baseline: `91abe94`);
...
|---|---|---|
| G0 source/compiler binding | PARTIAL | official compiler/source/executable hashes recorded; a validated engineering receipt is bound to the clean published commit `b3682abb30ec1a284398ad78df6df7e1dd471397` (`dirty=false`, `tree_sha256` recorded in the receipt); release tag still pending |
| G1 Sounio executable fixtures | PARTIAL | positional, streaming FASTA, and parameterized mini-pipeline fixtures (positional + masked k-mer k=1..8 with dual kernels and strict parameter validation) execute; fixture null-model engine (mononucleotide-preserving shuffle, schema 0.3.0) executes on two dedicated cases; complete-replicon smoke and product modes (`--replicon-profile`, `--exclusions`) execute on the frozen cohort; the metamorphic suite of spec §12.1 remains absent |
...
| G3 independent Julia differential validation | PARTIAL | operator, six FASTA cases, all four mini-pipeline JSONL artifacts (192/192 windows including both null-model cases, every shuffled draw identical), the complete-replicon smoke (207/207 windows), both plasmid window products (6,003/6,003 windows), and the three non-window products (4 profiles, 38 exclusions, cohort rebuild) reproduced byte-exact at tolerance 0; deterministic stratified atlas sample per spec §12.2 wired into the products runner (5/207 + 90/5,796 windows byte-exact, seed bound to the parameter hash) and applied to both chromosome-scale products in the Fase I benchmark (4,464/4,464 + 5,308/5,308 windows byte-exact, tolerance 0); full (non-sampled) recomputation of chromosome-scale products remains open |
| G4 schema/provenance/checksum closure | PARTIAL | receipt, window JSONL 0.3.0 (183 fields with the additive null block), pipeline parameter (with optional null keys), and the three product schemas exist; mini-pipeline input checksum closure verified (including parameter fixtures); frozen cohort checksum closure verified against official NCBI MD5s; a validated two-stage engineering receipt (plasmid-products scope) exists and is structurally re-checked in `make contract`; release-scope canonical artifacts and receipt pending |
...
3. Run the G5 scale benchmark and assemble the G6 release bundle; set the
   pilot `min_kmer_effective_count` and null-model parameters per proposed
   ADR-0002 (still proposed, not accepted).
