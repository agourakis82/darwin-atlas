...
| Manifest-directed Sounio work-unit contract | IMPLEMENTED WITH CAPABILITY REFUSALS | the manifest fixture still refuses unbound pilot execution; the bounded work-shard executor binds `run_id`, emits eight eligible profiles plus one exact `NULL_INPUT_NOT_ACGT` exclusion, proves a byte-exact resume suffix, and executes/reuses all six planned shards while retaining the zero-window unit as `PARTIAL_WINDOW`; there is no canonical pilot executor, chromosome-scale run, pilot output manifest, execution receipt or pilot evidence |
| Independent Julia manifest, multiscale-null and scientific secondary evaluators | IMPLEMENTED / synthetic fixtures | Julia independently regenerates 4000 exact Euler/Wilson draws and all 17 observed/null blocks in four multiscale rows, returns to manifest/FASTA inputs to validate all nine composition rows and the resume artifact, and now closes exact coverage over the complete six-shard logical set; Parquet/full-pilot validation is absent, held-out scientific scope remains explicit `REFUSE`, and no real scientific receipt exists |
...
A separate engineering capacity/profile fixture binds the canonical U0
parameter bytes and four deterministic synthetic cases at 16, 100, 500 and
1000 bp. Pinned Sounio performs 1000 fixed-endpoint Euler/Wilson draws per
case and emits both all 4000 draws and four complete nested v3 profile rows.
Each profile contains positional R/RC plus k-mer R (k=2..8) and RC (k=1..8),
with exact observed fractions, n=1000 mean/MAD/quantiles and tail counts. A
Base-only Julia implementation independently reconstructs every shuffle,
metric, summary and complete JSON line byte for byte; a stdlib structural
validator and the public JSON Schema reject malformed rows. The runner also
proves deterministic repetition and five zero-byte rc=12 refusal paths,
...
`687830a2b14e2f7ad9f54126acc674439152df9c15f3d726dc9e5b7990da4c4f`,
the compiled ELF is
`61442b174cf31df500e4df6fb9e69a7bb8c3d915744f42b8eaa3e3fc6556aaa5`,
the cases are
...
rehashing and reopening them; Julia independently recomputes all nine rows and
rejects a one-digit artifact perturbation. These are logical fixture artifacts,
not the gate's pilot output manifest, execution receipt, Parquet payload or
full-pilot Julia receipt.
...
| U250 dinucleotide_summaries kernel (Fase P2, hardware PASS) | PASS (engineering measurement) | Kernel v2 with jump-ahead ROM, per-window shared shuffle and in-kernel 18-metric exact integer summaries over the shared null core; xclbin v2 (Vitis 2025.1 on VM `vitis-u250-builder`, sha256 `62a3acb5...`) executed on the dl380 U250: frozen 8×8 anchor 720/720 summary fields bit-exact at tolerance 0, integrity 18,432/18,432 blocks; sweep with full per-draw pipeline: R=8 → 576 draws/s, R=64 → 4,033, R=256 → 11,278, R=1,024 → 20,447 (throughput now grows with R; ~19 cases/s at R=1,024 puts the n=1000 pilot at ~4 h/chromosome at this window scale). Receipt `receipts/u250-summaries-probe-20260808T204714Z/`; ADR-0002 annex §P2/§P2 hardware |
| Radeon AI PRO R9700 dinucleotide benchmark (Fase B3) | PASS (engineering measurement) | Same P2 shared core compiled HIP/ROCm 7.2.4, executed on `dl380-proxmox` (pod `beagle/rocm-r9700-compute`, gfx1201, PCIe 32 GT/s x16): anchor 8×8 draws and 720 summary fields bit-exact, then full-pipeline sweep — R=8: 198,351 draws/s; R=64: 1,762,387; R=256: 7,352,308; R=1,024: 28,003,204 draws/s (~344-3,483× the U250 P2 at the same scope; kernel2 single-thread-per-block bitonic summaries is the documented bottleneck, kernel1 alone reaches 166M draws/s at R=1,024). Receipt `receipts/r9700-dinucleotide-benchmark-20260808T184342Z/`; ADR-0002 annex §B3 |
| null_replicates ceiling 64 → 1000 (Fase P3) + null-primary decision D1 | PASS | Ceiling raised in schema (`maximum: 1000`), Sounio executable spec (`NULL_MAX_REPLICATES: i64 = 1000`, `NULL_VALS` buffer 1152→18000), independent Julia validator (`NULL_MAX_REPLICATES = 1000`) and Makefile contract (updated schema guard + two new grep guards pinning both engine constants). Recompiled binary (source `d89a7066...`, ELF `b3b1ac08...`) reproduces all 6/6 frozen mini-pipeline hashes byte-identical with opt==ref; end-to-end n=1000 run on null_k4 (12 windows) opt1==opt2==reference, artifact `b6f0d561...`, independently recomputed byte-exact by the new Base-only `validate_null_n1000.jl` (`DOSA_JULIA_NULL_N1000_DIFFERENTIAL_OK`, tolerance 0). Decision D1 (2026-08-08, ADR-0002): `dinucleotide_shuffle` is the sole pilot-primary null; `mononucleotide_shuffle` stays as legacy/sensitivity with no re-engineering (option C rejected — the mono engine already failed the Fase N uniformity gate structurally). Receipt `receipts/null-replicates-n1000-20260809T013016Z/` |
